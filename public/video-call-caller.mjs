...
async function removeEmergencyHistory(button) {
  const token = localStorage.getItem(KEY_TOKEN);
  if (!token) {
  ...
  const sender = messageBox.querySelector(".username").textContent.trim();
  const timestamp = messageBox.querySelector(".time").textContent.trim();
  const content = messageBox.querySelector(".message").textContent.trim();

  await deleteEmergencyHistory({ sender, timestamp, content, token });
...
async function populateEmergencyHistory() {
  ...
  try {
    const { history } = await getEmergencyHistory({ who: "self", token });
  ...
  deleteButtons.forEach((button) => {
    button.addEventListener("click", async (e) => {
      console.log("Clicked");
      await removeEmergencyHistory(button);
    });
  });
}
...
document.querySelector("#call").addEventListener("click", async (e) => {
  call();
  await populateEmergencyHistory();
});
